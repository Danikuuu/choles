...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["refund"])) {
    ...
    $fileName = basename($_FILES["refund"]["name"]);
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        $dbFilePath = "refund/" . $uniqueFileName; // Relative path for database
        $targetFilePath = $targetDir . $uniqueFileName; // Absolute path

        // Debug: Check if file exists before moving
        if (!file_exists($_FILES["refund"]["tmp_name"])) {
            die("File was not uploaded properly.");
        }

        // Debug: Display paths
        echo "Temp File: " . $_FILES["refund"]["tmp_name"] . "<br>";
        echo "Target File Path: " . $targetFilePath . "<br>";

        if (!move_uploaded_file($_FILES["refund"]["tmp_name"], $targetFilePath)) {
            die("Failed to move file. Error: " . error_get_last()["message"]);
        }

        // Update database
        $sql = "UPDATE reservations SET refund_img = ? WHERE id = ?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("si", $dbFilePath, $reservation_id);

        if ($stmt->execute()) {
            $_SESSION['success'] = "refund uploaded successfully!";
        } else {
